...
window.onload = function () {
	...
	function play(a){
		...
		P2select = Math.floor(Math.random()*3) + 1
		
		if(a==P2select){
			
		}else if(a==2 & P2select==1){
			scoreP1++
		}else if(a==1 & P2select==3){
			scoreP1++
		}else if(a==3 & P2select==1){
			scoreP1++
		}else{
			scoreP2++
		}
	...
	function getElements(){
		var newarr=[]
		var key
		for (key of ["start-game","menu-ingame","rock","paper","scissors","score","round","scoreboard"]){
